...
int main(int argc, char **argv)
{
	...
	int dir = 2;
	double *eps = (double *) rrl_calloc(nvoi, sizeof(double));
	double *sig = (double *) rrl_malloc(nvoi * sizeof(double));
	double *ctan = (double *) rrl_malloc(nvoi * nvoi * sizeof(double));

	double (*sig_test)[nvoi], (*ctan_test)[nvoi * nvoi];

	sig_test = (double (*)[nvoi]) rrl_malloc(nvoi * ngp * sizeof(double));
	ctan_test = (double (*)[nvoi * nvoi]) rrl_malloc(nvoi * nvoi * ngp * sizeof(double));

	micropp<dim> *micro = new micropp<dim>(ngp, size, micro_type, micro_params, mat_params);
